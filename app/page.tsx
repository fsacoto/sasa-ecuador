...
function IconSettings({ className = 'w-5 h-5 shrink-0' }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
      <path
        strokeLinecap="round"
...
function AppContent() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mt-auto px-1.5 py-2 space-y-1.5">
          <button
            type="button"
            onClick={() => {
              setStorefrontConfigOpen(true);
              setSuiteFlyout(null);
            }}
            className={`flex w-full items-center gap-1.5 rounded-md px-2 py-1.5 text-xs text-[#c5c5c5] transition-colors hover:bg-[#1a1a1a] hover:text-[#c5c5c5] ${
              sidebarCollapsed ? 'justify-center' : ''
            }`}
            title={t('common.storefrontConfig')}
          >
            <IconSettings className="h-4 w-4 shrink-0" />
            {!sidebarCollapsed && <span className="truncate">{t('common.storefrontConfig')}</span>}
          </button>
          <button
            type="button"
            onClick={() => {
      ...
      <StorefrontConnectionPanel
        open={storefrontConfigOpen}
        onClose={() => setStorefrontConfigOpen(false)}
      />
