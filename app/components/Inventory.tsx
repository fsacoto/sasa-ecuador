...
export default function Inventory({ darkMode = false }: InventoryProps) {
  ...
  const handleSubmit = async (e: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      let resolvedSku = skuValidation.sku;
      if (!skuValidation.ok) {
        if (skuValidation.reason === 'supplier_mismatch') {
          if (skuManuallyEdited && formData.sku.trim()) {
            // User regenerated/edited SKU (e.g. EO→LO); keep it and sync to linked POs.
            resolvedSku = formData.sku.trim();
          } else {
            alert(
  ...
  const handleRegenerateSku = () => {
    ...
    // Always build from categoría + línea (ignore supplier link / read-only typing).
                ...
                <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      // EO↔LO fallback on last known URL
                      const last = urls[Math.min(idx, urls.length - 1)] || '';
                      const swapped = last.replace(
                        /([A-Za-z]{2})(EO|LO)(\d{4}(?:-\d+)?)/i,
                        (_m, p, mid, seq) =>
                          `${String(p).toUpperCase()}${String(mid).toUpperCase() === 'EO' ? 'LO' : 'EO'}${seq}`
                      );
                      if (swapped && swapped !== img.src && !img.getAttribute('data-swapped')) {
                      ...
                      if (parent && !parent.querySelector('[data-img-fallback]')) {
                        ...
                        ph.setAttribute('data-img-fallback', '1');
                        ph.className =
                      ...
                      <div key={field.key} className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`toggle-switch relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#515151] focus:ring-offset-2 ${
                            galleryFields.has(field.key) ? 'toggle-switch-on' : 'toggle-switch-off'
                          }`}
                        >
                          <span
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto max-h-[calc(90vh-8rem)] p-6 space-y-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p className="text-xs text-gray-500 mt-2">
                  {editingItem
                    ? t('inventory.regenerateSkuEditHint')
                    : t('inventory.autoGeneratesWhen')}
                </p>
              </div>
